...
let groupedHeatLocations = new Map(); // Keyed by normalized lat,lng

function getLocationKey(lat, lng) {
    return `${lat.toFixed(6)},${lng.toFixed(6)}`;
}

function groupSubmissionsByLocation(validSubmissions) {
    ...
    validSubmissions.forEach((s) => {
        ...
        const carbon = parseFloat(s.carbon) || 0;
    ...
    return Array.from(grouped.values()).map((group) => ({
        ...group,
        avgCO2: group.totalTrees > 0 ? group.totalCO2 / group.totalTrees : 0,
        avgCarbon: group.totalTrees > 0 ? group.totalCarbon / group.totalTrees : 0
    }));
}
        ...
        <tr>
            ...
            <td>
                <div style="font-size: 12px; line-height: 1.35;">
                    <div><strong>${sub.method === 'method2' ? 'M2' : 'M1'}</strong></div>
                    <div style="opacity: 0.8;">${sub.forestType ? (sub.forestType.charAt(0).toUpperCase() + sub.forestType.slice(1)) : 'N/A'}</div>
                </div>
            </td>
...
window.viewDetails = async (submissionId) => {
    ...
    try {
        ...
        const forestTypeLabel = data.forestType
            ? data.forestType.charAt(0).toUpperCase() + data.forestType.slice(1)
            : 'N/A';
        const methodLabel = data.methodLabel || data.method || 'N/A';
        const carbonFraction = data.carbonFraction ? data.carbonFraction : 'N/A';
        const woodDensityValue = data.woodDensity ? `${data.woodDensity} g/cm3` : 'N/A';
        const heightUsedValue = data.heightUsed ? `${data.heightUsed} m` : 'Not used';
        ...
        modalBody.innerHTML = `
            ...
            <div class="detail-row">
                ...
                <span>${woodDensityValue}${data.woodDensitySource ? ` (${data.woodDensitySource})` : ''}</span>
            </div>
            <div class="detail-row">
                <strong>Height Used:</strong>
                <span>${heightUsedValue}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                <strong>Carbon Fraction:</strong>
                <span>${carbonFraction}</span>
            </div>
            <div class="detail-row">
...
// Switch Tabs
window.switchTab = (tabName, clickedBtn = null) => {
    // Update buttons
...
function updateHeatMap(submissions) {
    ...
    // Group trees recorded at the same location and aggregate their impact.
    const groupedLocations = groupSubmissionsByLocation(validSubmissions);
    groupedHeatLocations = new Map(groupedLocations.map((g) => [g.key, g]));
    console.log(`📌 Grouped into ${groupedLocations.length} unique map locations`);
    ...
    // Calculate max aggregated CO2 for normalization
    const maxCO2 = Math.max(...groupedLocations.map(g => g.totalCO2));
    console.log(`📊 Max aggregated CO2 value: ${maxCO2} kg`);
    
    // Prepare heat data with normalized intensity
    const heatData = groupedLocations.map(g => {
        const intensity = maxCO2 > 0 ? g.totalCO2 / maxCO2 : 0.5; // Normalize 0-1
        console.log(`🔥 Heat point: [${g.lat}, ${g.lng}] Trees: ${g.totalTrees}, Total CO2: ${g.totalCO2.toFixed(2)} kg, intensity: ${intensity.toFixed(2)}`);
    ...
    groupedLocations.forEach(g => {
        const lat = g.lat;
        const lng = g.lng;
        const co2Value = g.totalCO2;
        ...
        if (co2Value < maxCO2 * 0.3) markerColor = '#27ae60'; // Green - Low
                ...
                L.circleMarker([lat, lng], {
                        radius: Math.min(14, 6 + g.totalTrees),
          ...
          .bindPopup(`
                            ...
                            Total Trees: <strong>${g.totalTrees}</strong><br>
                            Total CO2: <strong>${g.totalCO2.toFixed(2)} kg</strong><br>
                            Avg CO2 / Tree: <strong>${g.avgCO2.toFixed(2)} kg</strong><br>
                            Avg Carbon / Tree: <strong>${g.avgCarbon.toFixed(2)} kg C</strong><br>
                            Location: ${lat.toFixed(4)}, ${lng.toFixed(4)}<br><br>
                            <button
                                type="button"
                                onclick="showTreesAtLocation('${g.key}')"
                                style="padding:6px 10px;border:none;border-radius:8px;background:#10b981;color:#fff;font-weight:600;cursor:pointer;">
                                Show Trees
                            </button>
          `);
    });
    ...
    const bounds = L.latLngBounds(groupedLocations.map(g => [g.lat, g.lng]));
    heatMapInstance.fitBounds(bounds, { padding: [50, 50], maxZoom: 15 });
    
    console.log('✅ Heat map updated with', groupedLocations.length, 'grouped locations');
}

window.showTreesAtLocation = (locationKey) => {
    const group = groupedHeatLocations.get(locationKey);
    if (!group) {
        alert('No tree details found for this location.');
        return;
    }
    ...
    if (modalTitle) {
        modalTitle.innerHTML = '<i class="fas fa-map-marker-alt"></i> Trees At Selected Location';
    }

    const rows = group.trees.map((tree, idx) => {
        const treeName = tree.treeName || 'Unknown';
        const studentId = tree.studentId || 'N/A';
        const carbon = parseFloat(tree.carbon) || 0;
        const co2 = parseFloat(tree.co2) || 0;
        const method = tree.method === 'method2' ? 'M2' : 'M1';
        return `
            <div class="detail-row">
                <strong>#${idx + 1} ${treeName}</strong>
                <span>${studentId} | ${method} | C ${carbon.toFixed(2)} kg | CO2 ${co2.toFixed(2)} kg</span>
            </div>
        `;
    }).join('');
    ...
    if (modalBody) {
        modalBody.innerHTML = `
            <div class="detail-row">
                ...
                <span>${group.lat.toFixed(6)}, ${group.lng.toFixed(6)}</span>
            </div>
            <div class="detail-row">
                <strong>Total Trees</strong>
                <span>${group.totalTrees}</span>
            </div>
            <div class="detail-row">
                <strong>Total CO2</strong>
                <span>${group.totalCO2.toFixed(2)} kg</span>
            </div>
            <div class="detail-row">
                <strong>Average CO2 / Tree</strong>
                <span>${group.avgCO2.toFixed(2)} kg</span>
            </div>
            <hr style="margin: 14px 0; border-color: rgba(255,255,255,0.1);">
            <h3 style="color: #a78bfa; margin-bottom: 10px;">Individual Trees</h3>
            ${rows}
        `;
    }
